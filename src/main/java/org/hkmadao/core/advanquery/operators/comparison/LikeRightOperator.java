...
public class LikeRightOperator implements IOperator {

    private LikeRightOperator() {
    }

    private static LikeRightOperator instance = new LikeRightOperator();

    public static LikeRightOperator getInstance() {
        return instance;
    }
    ...
    public <T extends BaseEntity> Predicate createPredicate(Map<String, From<T, T>> fromMap, Root<T> root,
        ...
        if (!fullFieldName.contains(".")) {
            predicate = criteriaBuilder.like(root.get(aqFilterNode.getName()), aqFilterNode.fetchFirstParam().toString() + "%");
            return predicate;
        }
        ...
        predicate = criteriaBuilder.like(fromMap.get(joinTableName).get(fieldName), aqFilterNode.fetchFirstParam().toString() + "%");
        return predicate;
    }
}
